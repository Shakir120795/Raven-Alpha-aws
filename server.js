...
async function runScheduledScan() { const nft = await runNftScan(); const ethereumToken = await runEthereumTokenScan(); const robinhoodToken = await runRobinhoodTokenScan(); if (nft.skipped) console.log(`[${new Date().toISOString()}] NFT scan skipped:`, nft.reason); else console.log(`[${new Date().toISOString()}] NFT scan done:`, nft.walletsScanned || 0, "wallets,", nft.nftMints || 0, "new mints,", nft.alertsSent || 0, "collection alerts"); if (ethereumToken?.skipped || ethereumToken?.disabled) console.log(`[${new Date().toISOString()}] Ethereum token scan skipped:`, ethereumToken.reason || "disabled"); else console.log(`[${new Date().toISOString()}] Ethereum token scan:`, ethereumToken?.walletsScanned || 0, "wallets,", ethereumToken?.tokenTransfers || 0, "incoming ERC20 transfers,", ethereumToken?.tokenAlerts || 0, "token alerts"); if (robinhoodToken?.skipped) console.log(`[${new Date().toISOString()}] Robinhood token scan skipped:`, robinhoodToken.reason); else console.log(`[${new Date().toISOString()}] Robinhood token scan:`, robinhoodToken?.walletsScanned || 0, "wallets,", robinhoodToken?.tokenTransfers || 0, "incoming ERC20 transfers,", robinhoodToken?.tokenAlerts || 0, "token alerts"); }
cron.schedule("*/5 * * * *", async () => { console.log(`[${new Date().toISOString()}] Running NFT + Ethereum + Robinhood token scan...`); try { await runScheduledScan(); } catch (e) { console.error("NFT/Arc/Robinhood token scan failed:", e.message); } });
