...
app.post("/api/scan", requireSecret, async (req, res) => { try { const nft = await runNftScan(); const arcToken = await runArcTokenScan(); const robinhoodToken = await runRobinhoodTokenScan(); res.json({ ok: true, nft, arcToken, robinhoodToken }); } catch (e) { res.status(500).json({ ok: false, error: e.message }); } });
app.listen(PORT, () => console.log(`🍌 Raven Alpha server running on port ${PORT}`));
async function runScheduledScan() { const nft = await runNftScan(); const arcToken = await runArcTokenScan(); const robinhoodToken = await runRobinhoodTokenScan(); if (nft.skipped) console.log(`[${new Date().toISOString()}] NFT scan skipped:`, nft.reason); else console.log(`[${new Date().toISOString()}] NFT scan done:`, nft.walletsScanned || 0, "wallets,", nft.nftMints || 0, "new mints,", nft.alertsSent || 0, "collection alerts"); if (arcToken?.skipped) console.log(`[${new Date().toISOString()}] Arc token scan skipped:`, arcToken.reason); else console.log(`[${new Date().toISOString()}] Arc token scan:`, arcToken?.walletsScanned || 0, "wallets,", arcToken?.tokenTransfers || 0, "incoming ERC20 transfers,", arcToken?.tokenAlerts || 0, "token alerts"); if (robinhoodToken?.skipped) console.log(`[${new Date().toISOString()}] Robinhood token scan skipped:`, robinhoodToken.reason); else console.log(`[${new Date().toISOString()}] Robinhood token scan:`, robinhoodToken?.walletsScanned || 0, "wallets,", robinhoodToken?.tokenTransfers || 0, "incoming ERC20 transfers,", robinhoodToken?.tokenAlerts || 0, "token alerts"); }
cron.schedule("*/5 * * * *", async () => { console.log(`[${new Date().toISOString()}] Running NFT + Arc + Robinhood token scan...`); try { await runScheduledScan(); } catch (e) { console.error("NFT/Arc/Robinhood token scan failed:", e.message); } });
