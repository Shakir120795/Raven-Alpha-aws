...
function createTokenScanner({ chain, enabledEnv, rpcEnvVars = [], fallbackRpc, label, transferScanMode = 'receipts', logWalletBatchSize = 500, logBlockSpan = 500 }) {
  ...
  async function classify(tx, receipt, walletAddress, targetToken, block) {
    if (!tx || !receipt) return { action: null, reason: 'missing_tx_or_receipt' };
    if (String(receipt.status || '0x1') === '0x0') return { action: null, reason: 'failed_tx' };
    const wallet = walletAddress.toLowerCase();
    ...
    if (!targetIn && !targetOut) return { action: null, reason: 'target_transfer_missing_in_receipt' };
    ...
    if (targetIn && !targetOut) {
      if (otherOut || nativeOut) return { action: 'buy', reason: 'buy_payment_found' };
      return { action: null, reason: 'target_in_no_payment' };
    }

    if (targetOut && !targetIn) {
      if (otherIn) return { action: 'sell', reason: 'sell_token_payment_found' };
      if (await nativeGainCoversGas(tx, receipt, wallet, block)) return { action: 'sell', reason: 'sell_native_payment_found' };
      return { action: null, reason: 'target_out_no_payment' };
    }

    if (targetIn && targetOut) {
      if (otherOut || nativeOut) return { action: 'buy', reason: 'buy_payment_found_roundtrip' };
      if (otherIn) return { action: 'sell', reason: 'sell_payment_found_roundtrip' };
      if (await nativeGainCoversGas(tx, receipt, wallet, block)) return { action: 'sell', reason: 'sell_native_payment_roundtrip' };
      return { action: null, reason: 'roundtrip_no_payment' };
    }
    return { action: null, reason: 'unclassified' };
  }
  ...
  async function run() {
    ...
    try {
      ...
      for (const row of fresh) {
        ...
        if (!tx) {
          rejectReasons.missing_tx = (rejectReasons.missing_tx || 0) + 1;
          continue;
        }
        ...
        if (!receipt) {
          rejectReasons.missing_receipt = (rejectReasons.missing_receipt || 0) + 1;
          continue;
        }

        seen[row.key] = Date.now();
        const result = await classify(tx, receipt, row.wallet.address, row.contract, row.block);
        if (!result.action) {
          rejectReasons[result.reason] = (rejectReasons[result.reason] || 0) + 1;
          continue;
        }
        confirmedTrades++;
        if (await sendTradeAlert(row, result.action)) tokenAlerts++;
      }
      ...
      store.set(seenKey, seen);
      console.log(`[${label} TOKEN] scanned ${wallets.length} wallets, ${rawTransfers} transfer events, ${fresh.length} candidates, ${confirmedTrades} confirmed trades, ${tokenAlerts} alerts, cursor=${currentBlock}` + (Object.keys(rejectReasons).length ? ', rejects=' + JSON.stringify(rejectReasons) : ''));
