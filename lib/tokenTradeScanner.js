...
function createTokenScanner({ chain, enabledEnv, rpcEnvVars = [], rpcKeyEnvConfigs = [], fallbackRpc, label, blockscoutApi = null }) {
  ...
  async function blockscoutGet(path, params = {}) {
    ...
    try {
      ...
      const r = await fetch(url, { signal: AbortSignal.timeout(RPC_TIMEOUT_MS), headers: { 'user-agent': 'Raven-Alpha/1.0' } });
