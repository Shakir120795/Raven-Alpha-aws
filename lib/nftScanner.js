...
async function rpc(chain, method, params) {
  ...
  for (let attempt = 1; attempt <= RPC_RETRIES; attempt++) {
    try {
      ...
      if (method === 'eth_blockNumber' && attempt > 1) {
        console.log(`[RPC OK] ${chain.id} ${method} after retry ${attempt}/${RPC_RETRIES}`);
      }
      return j.result;
    } catch (e) {
      lastError = e?.message || lastError;
      if (attempt < RPC_RETRIES) await new Promise(resolve => setTimeout(resolve, RPC_BACKOFF_MS * attempt));
    }
  }
...
async function getLogs(chain, fromBlock, toBlock, topics) {
  const result = await rpc(chain, 'eth_getLogs', [{ fromBlock: '0x' + fromBlock.toString(16), toBlock: '0x' + toBlock.toString(16), topics }]);
...
async function sendBatch(chain, entry) {
  ...
  const embed = {
    title: '🟣 NEW NFT MINT',
    description: [
      `**${entry.name}** · ${chain.id.toUpperCase()}`,
      '',
      explorer && `[🔎 Explorer](${explorer})`,
      extra.mintPage && `[🪙 Mint Page](${extra.mintPage})`,
      extra.x && `[𝕏 X](${extra.x})`,
...
async function scanRpcChain(chain, wallets) {
  ...
  if (latest == null) {
    console.error(`[NFT RPC] ${chain.id}: latest block unavailable; keeping cursor unchanged`);
    return null;
  }
  ...
  for (let start = from; start <= latest; start += blockSpan) {
    ...
    for (let i = 0; i < walletTopics.length; i += walletBatchSize) {
      ...
      if (erc721 == null || single == null || batch == null) {
        console.error(`[NFT RPC] ${chain.id}: eth_getLogs failed for blocks ${start}-${end}, wallet batch ${Math.floor(i / walletBatchSize) + 1}; cursor NOT advanced`);
        return null;
      }
  ...
  console.log(`[NFT RPC] ${chain.id}: OK — ${rpcRequests} log requests, ${found.length} mint events, cursor=${latest}`);
  return found;
}
...
async function scanExplorerChain(chain, wallets) {
  ...
  for (const w of wallets) {
    const key = keys.length ? keys[keyIndex++ % keys.length] : null;
    const qs = new URLSearchParams({ module: 'account', action: 'tokennfttx', address: w.address, sort: 'desc', offset: '100', ...(chain.extraParams || {}), ...(key ? { apikey: key } : {}) });
    const res = await jsonFetch(`${chain.api}?${qs}`);
    if (res?.status === '0' && res?.message === 'NOTOK') console.warn(`[NFT Explorer] ${chain.id}: API NOTOK for ${w.address}: ${res?.result || ''}`);
...
async function runNftScan() {
  ...
  try {
    ...
    const alerts = [];
    if (!baselineDone) {
      store.set('nft_baseline_v2', true);
      console.log(`[${new Date().toISOString()}] NFT baseline created: ${discovered} historical/recent mint events recorded without alerts`);
    } else {
