...
async function metadataLinks(chain, contract, tokenId, is1155) {
  ...
  const result = { mintPage: external && /^https?:\/\//i.test(external) ? external : null, x: x && /^https?:\/\//i.test(x) ? x : (collection ? `https://x.com/search?q=${encodeURIComponent(collection)}&f=live` : null) };
...
async function sendBatch(chain, entry) {
  ...
  const links = [explorer && `<a href="${explorer}">Explorer</a>`, extra.mintPage && `<a href="${extra.mintPage}">Mint Page</a>`, extra.x && `<a href="${extra.x}">X</a>`, openSea && `<a href="${openSea}">OpenSea</a>`, collectionPage && `<a href="${collectionPage}">Collection</a>`].filter(Boolean).join(' · ');
  ...
  const alert = { type: 'wallet_nft_mint_batch', chain: chain.id, contract: entry.contract, nftName: entry.name, mintedThisCycle: entry.minted, wallets, collectionSupply: total, maxSupply: max, remainingSupply: remaining, links: { explorer, mintPage: extra.mintPage || null, x: extra.x || null, openSea, collection: collectionPage }, result: results, at: new Date().toISOString() };
