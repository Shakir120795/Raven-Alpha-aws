...
async function sendBatch(chain, entry) {
  ...
  const dsHook = process.env.DISCORD_NFT_WEBHOOK;
...
async function scanRpcChain(chain, wallets) {
  ...
  const walletBaselines = store.get(`nft_wallet_baselines_v1_${chain.id}`, {});
  const baselineValues = wallets.map(w => Number(walletBaselines[w.id || w.address.toLowerCase()] || 0)).filter(Boolean);
  const minimumWalletBaseline = baselineValues.length ? Math.min(...baselineValues) : 0;
  // Ignore a stale global cursor when wallet baselines are already current.
  // This prevents Robinhood from replaying millions of historical blocks.
  const lastBlock = last == null ? 0 : Number(last);
  const staleCursor = lastBlock > 0 && latest - lastBlock > 10000;
  const from = staleCursor
    ? Math.max(0, latest - 20)
    : last == null
      ? Math.max(0, minimumWalletBaseline || latest - 20)
      : Math.max(0, lastBlock + 1, minimumWalletBaseline || 0);
...
async function runNftScan() {
  ...
  try {
    ...
    for (const chain of CHAINS) {
      if (chain.comingSoon || chain.id === "arc") continue;
      const ws = byChain[chain.id] || [];
      if (!ws.length) continue;
      const walletBaselines = store.get(`nft_wallet_baselines_v1_${chain.id}`, {});
      const latestForBaseline = await getLatest(chain);
      if (latestForBaseline != null) {
        let baselineChanged = false;
        for (const wallet of ws) {
          const id = wallet.id || wallet.address.toLowerCase();
          if (walletBaselines[id] == null) { walletBaselines[id] = latestForBaseline; baselineChanged = true; }
        }
        if (baselineChanged) store.set(`nft_wallet_baselines_v1_${chain.id}`, walletBaselines);
      }
      const items = chain.useRpcLogs ? await scanRpcChain(chain, ws) : await scanExplorerChain(chain, ws);
      if (items == null) continue;
      for (const item of items) {
        const walletBaseline = Number(walletBaselines[item.wallet.id || item.wallet.address.toLowerCase()] || 0);
        if (item.block <= walletBaseline) continue;
