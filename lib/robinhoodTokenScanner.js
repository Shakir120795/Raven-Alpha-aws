...
const NATIVE_ROH = (process.env.ROBINHOOD_NATIVE_TOKEN || '').toLowerCase();
...
const seenKey = 'robinhood_token_seen_v3';
const cursorKey = 'robinhood_token_last_block_v3';
...
function envEnabled() {
  return String(process.env.ROBINHOOD_TOKEN_TRACKING_ENABLED || 'false').toLowerCase() === 'true';
}
...
async function rpc(method, params) {
  ...
  console.error(`[RH TOKEN RPC FAILED] ${method}: ${lastError}`);
  return null;
}
...
function hasBuyEvidence(tx, receipt, walletAddress, boughtToken) {
  ...
  for (const log of Array.isArray(receipt.logs) ? receipt.logs : []) {
    ...
    if (from === wallet && to !== ZERO && token !== target && token !== NATIVE_ROH) paymentOut = true;
  }
  try {
...
async function runRobinhoodTokenScan() {
  if (!envEnabled()) return { disabled: true, reason: 'ROBINHOOD_TOKEN_TRACKING_ENABLED=false' };
  ...
  try {
    ...
    if (!cursor) {
      ...
      console.log(`[RH TOKEN] RPC baseline initialized at block ${currentBlock}; ${wallets.length} tracked wallets, historical buys ignored`);
    ...
    store.set(seenKey, seen);
    console.log(`[RH TOKEN] RPC scanned ${wallets.length} tracked wallets, ${rawTransfers} incoming ERC20 transfers, ${fresh.length} candidates, ${confirmedBuys} confirmed buys, ${tokenAlerts} token alerts, cursor=${currentBlock}`);
