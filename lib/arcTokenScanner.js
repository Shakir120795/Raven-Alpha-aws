...
const WALLET_CONCURRENCY = 20;
const MAX_ADDRESS_PAGES = 2;
...
function buildActivityPath(address, cursor = null) {
  const params = new URLSearchParams({ limit: String(API_PAGE_SIZE) });
  if (cursor) params.set('cursor', cursor);
  return `/address/${encodeURIComponent(String(address).toLowerCase())}/activity?${params.toString()}`;
}

async function scanWalletActivity(wallet) {
  let cursor = null;
  const rows = [];
  let pages = 0;
  let newestBlock = 0;
  while (pages < MAX_ADDRESS_PAGES) {
    const payload = await apiGet(buildActivityPath(wallet.address, cursor), true);
    if (!payload) return { ok: false, rows, newestBlock, pages };
    const items = unwrapItems(payload);
    if (!items.length) break;
    pages++;
    for (const raw of items) {
      const row = normalizeTransfer(raw);
      newestBlock = Math.max(newestBlock, row.block || 0);
      rows.push(row);
    }
  ...
  return { ok: true, rows, newestBlock, pages };
}
...
async function runArcTokenScan() {
  ...
  try {
    ...
    const cursor = Number(store.get('arc_token_last_block_v9', 0) || 0);
    const seen = store.get('arc_token_seen_v9', {});
    ...
    const results = await mapWithConcurrency(wallets, WALLET_CONCURRENCY, scanWalletActivity);
    const workingResults = results.filter(r => r?.ok);
    const apiWorking = workingResults.length > 0;
    const rows = [];
    let newestIndexedBlock = cursor;
    for (const result of workingResults) {
      newestIndexedBlock = Math.max(newestIndexedBlock, result.newestBlock || 0);
      rows.push(...result.rows);
    }

    if (!apiWorking) {
      console.warn('[ARC TOKEN] ArcScan address activity API unavailable; cursor NOT advanced');
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, error: 'ArcScan address activity API unavailable', baselineReady: cursor > 0 };
    }

    // First successful read establishes a live baseline only; never backfill historical buys.
    if (!cursor) {
      if (newestIndexedBlock > 0) store.set('arc_token_last_block_v9', newestIndexedBlock);
      store.set('arc_token_seen_v9', seen);
      console.log(`[ARC TOKEN] baseline initialized from ${workingResults.length}/${wallets.length} address feeds at block ${newestIndexedBlock}; no historical token alerts`);
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, baselineReady: true, cursor: newestIndexedBlock, walletsWithApi: workingResults.length };
    }

    const unique = new Map();
    for (const raw of rows) {
      const row = normalizeTransfer(raw.raw || raw);
    ...
    store.set('arc_token_last_block_v9', newestIndexedBlock);
    store.set('arc_token_seen_v9', seen);
    ...
    console.log(`[ARC TOKEN] scanned ${wallets.length} wallets via ${workingResults.length} activity feeds, ${unique.size} new tracked ERC20 candidates, ${buyCandidateCount} buy candidates, ${buyConfirmedCount} confirmed buys, ${tokenAlerts} token alerts, cursor=${newestIndexedBlock}`);
    return { walletsScanned: wallets.length, tokenAlerts, tokenTransfers: unique.size, buyCandidates: buyCandidateCount, confirmedBuys: buyConfirmedCount, baselineReady: true, walletsWithApi: workingResults.length, cursor: newestIndexedBlock };
  } finally {
    running = false;
  }
}
