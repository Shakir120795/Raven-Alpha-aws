...
async function fetchChainTransferPages(stopBlock) {
  ...
  return { rows, pages, apiWorking, reachedStopBlock, oldestBlock: oldestBlock === Number.MAX_SAFE_INTEGER ? stopBlock : oldestBlock, newestBlock };
}
...
async function runArcTokenScan() {
  ...
  try {
    ...
    // First successful chain-wide feed call establishes the baseline only.
    // We do not backfill historical token buys into alerts.
    if (!cursor) {
    ...
    if (!feed.reachedStopBlock && feed.rows.length >= API_PAGE_SIZE * API_MAX_PAGES) {
      console.warn('[ARC TOKEN] feed page limit reached before cursor; cursor NOT advanced to avoid missing transfers');
      store.set('arc_token_seen_v8', seen);
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: buyCandidateCount, buyCandidates: buyCandidateCount, confirmedBuys: buyConfirmedCount, baselineReady: true, pages: feed.pages, warning: 'feed page limit reached before cursor' };
    }
    ...
    console.log(`[ARC TOKEN] scanned ${wallets.length} wallets via chain feed, ${feed.pages} API pages, ${buyCandidateCount} buy candidates, ${buyConfirmedCount} confirmed buys, ${tokenAlerts} token alerts, cursor=${newestIndexedBlock}`);
    return { walletsScanned: wallets.length, tokenAlerts, tokenTransfers: buyCandidateCount, buyCandidates: buyCandidateCount, confirmedBuys: buyConfirmedCount, baselineReady: true, pages: feed.pages, cursor: newestIndexedBlock };
  } finally {
    running = false;
  }
}
