...
async function sendArcTokenBatch(entry) {
  ...
  // Arc is not guaranteed to have a DEX Screener indexed pair at alert time.
  // Keep the direct pair URL when the API knows one; otherwise always provide
  // a DexScreener search link for the exact contract so every Arc token alert
  // has a usable market link.
