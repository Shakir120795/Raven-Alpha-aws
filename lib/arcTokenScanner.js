...
async function sendArcTokenBatch(entry) {
  ...
  // Arc is not guaranteed to have an indexed DEX Screener pair at alert time.
  // Keep the direct pair URL when available; otherwise always provide a
  // DexScreener contract-search link so every Arc token alert has a market link.
  ...
  const enrichedMarketData = {
    ...(marketData || {}),
    dex: marketData?.dex || 'DexScreener',
    url: marketLink,
  };
