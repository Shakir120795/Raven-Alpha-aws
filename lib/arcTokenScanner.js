...
async function runArcTokenScan() {
  ...
  try {
    ...
    const seen = store.get('arc_token_seen_v4', {});
    ...
    // API path is intentionally wallet-scoped so we do not depend on chain-wide eth_getLogs.
    let rows = [];
    let nextCursor = null;
    let apiWorking = false;
    let newestIndexedBlock = cursor;

    // ArcScan account token-transfer endpoints are tried per tracked wallet. Different ArcScan
    // deployments expose one of these equivalent paths; the first path returning data wins.
    const paths = address => [
      `/address/${address}/token-transfers?limit=${API_PAGE_SIZE}`,
      `/account/${address}/token-transfers?limit=${API_PAGE_SIZE}`,
      `/explore/address/${address}/token-transfers?limit=${API_PAGE_SIZE}`,
    ];

    let walletQueries = 0;
    for (const wallet of wallets) {
      let walletRows = [];
      for (const path of paths(wallet.address)) {
        const payload = await apiGet(path);
        const candidate = unwrapItems(payload).map(normalizeTransfer).filter(r => r.block > 0);
        if (candidate.length) {
          walletRows = candidate;
          apiWorking = true;
          break;
        }
        // An empty successful response is still a valid endpoint; keep trying only when no data.
      }
      walletQueries++;
      if (!walletRows.length) continue;

      for (const row of walletRows) {
        if (row.block <= cursor || row.block <= 0) continue;
        ...
        if (row.block > newestIndexedBlock) newestIndexedBlock = row.block;
        rows.push(row);
      }
    }

    if (!apiWorking) {
      console.warn('[ARC TOKEN] ArcScan wallet token-transfer API unavailable; no token alerts generated');
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, error: 'ArcScan wallet token-transfer API unavailable', baselineReady: true };
    }

    // Newest-first account feeds can overlap heavily across scans; dedupe by tx/log/wallet/token.
    ...
    for (const { row, wallet, eventId } of unique.values()) {
      ...
      const confirmed = await hasBuyEvidence(row.txHash, wallet.address, row.token);
    ...
    if (newestIndexedBlock > cursor) store.set('arc_token_last_block', newestIndexedBlock);
    store.set('arc_token_seen_v4', seen);
    ...
    console.log(`[ARC TOKEN] scanned ${wallets.length} wallets, ${walletQueries} wallet queries, ${transferCount} incoming ERC20 transfers, ${buyCandidateCount} buy candidates, ${buyConfirmedCount} confirmed buys, ${tokenAlerts} token alerts, cursor=${newestIndexedBlock}`);
    return {
