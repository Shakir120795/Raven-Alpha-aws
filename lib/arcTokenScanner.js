...
async function runArcTokenScan() {
  ...
  try {
    ...
    if (!cursor) {
      ...
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, baselineReady: true, cursor: baseline };
    }
    ...
    const reportedCursor = Number(store.get('arc_token_last_block', cursor) || cursor);
    console.log(`[ARC TOKEN] chain feed: ${feed.pages} pages, ${feed.rows.length} new rows, ${buyCandidateCount} buy candidates, ${buyConfirmedCount} confirmed buys, ${tokenAlerts} token alerts, cursor=${reportedCursor}`);
    return { walletsScanned: wallets.length, tokenAlerts, tokenTransfers: feed.rows.length, buyCandidates: buyCandidateCount, confirmedBuys: buyConfirmedCount, pages: feed.pages, reachedCursor: feed.reachedStopBlock, baselineReady: true, cursor: reportedCursor };
  } finally {
    running = false;
  }
}
