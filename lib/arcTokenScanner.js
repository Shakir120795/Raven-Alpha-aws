...
async function runArcTokenScan() {
  ...
  try {
    ...
    if (!latestHex) {
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, error: 'latest block unavailable' };
    }
    ...
    const oldSeenKeys = Object.keys(seen);
    if (oldSeenKeys.length > 50000) {
      for (const key of oldSeenKeys.slice(0, oldSeenKeys.length - 50000)) delete seen[key];
    }
    ...
    let lookupCount = 0;
    let walletQueries = 0;
    let newestIndexedBlock = Number(cursor);
    ...
    for (const wallet of wallets) {
      ...
      for (let page = 0; page < MAX_API_PAGES; page++) {
        ...
        const payload = await apiGet(`/filter/token-transfers?${params.toString()}`);
        const rows = unwrapItems(payload).map(normalizeTransfer).filter(r => r.block > 0);
        walletQueries++;

        if (payload !== null) apiWorking = true;
    ...
    if (!apiWorking) {
      console.warn('[ARC TOKEN] ArcScan token-transfer filter API unavailable; no token alerts generated');
      return {
        walletsScanned: wallets.length,
        tokenAlerts: 0,
        tokenTransfers: 0,
        error: 'ArcScan token-transfer filter API unavailable',
        baselineReady: true,
      };
    }
    ...
    console.log(`[ARC TOKEN] scanned ${wallets.length} wallets, ${walletQueries} API queries, ${transferCount} incoming ERC20 transfers, ${buyCandidateCount} buy candidates, ${buyConfirmedCount} confirmed buys, ${tokenAlerts} token alerts, cursor=${newestIndexedBlock}`);
    return {
      walletsScanned: wallets.length,
      tokenAlerts,
