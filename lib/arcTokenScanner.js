...
async function fetchChainTransferPages(stopBlock) {
  ...
  while (pages < API_MAX_PAGES) {
    ...
    if (pageOldest !== Number.MAX_SAFE_INTEGER && pageOldest <= stopBlock) break;
    const next = pickNextCursor(payload);
    if (!next || next === cursor) break;
    cursor = next;
  }

  return { rows, pages, apiWorking, oldestBlock: oldestBlock === Number.MAX_SAFE_INTEGER ? stopBlock : oldestBlock, newestBlock };
}
...
async function runArcTokenScan() {
  ...
  try {
    ...
    console.log(`[ARC TOKEN] chain feed pages=${feed.pages}, tracked wallets=${wallets.length}, candidate transfers=${unique.size}, new candidates=${buyCandidateCount}, confirmed buys=${buyConfirmedCount}, token alerts=${tokenAlerts}, cursor=${newestIndexedBlock}`);
    return {
