...
function buildTransferFilter(address, extra = {}) {
  const params = new URLSearchParams({ address, role: 'to', standard: 'ERC-20', limit: String(API_PAGE_SIZE), ...extra });
  return `/filter/token-transfers?${params.toString()}`;
}

async function scanWalletTransfers(wallet, cursor) {
  const queries = [
    buildTransferFilter(wallet.address, { from_block: String(cursor + 1) }),
    buildTransferFilter(wallet.address, { fromBlock: String(cursor + 1) }),
    buildTransferFilter(wallet.address),
  ];
  for (const path of queries) {
    const payload = await apiGet(path, false);
    if (!payload) continue;
    const rows = unwrapItems(payload).map(normalizeTransfer).filter(r => r.block > cursor);
    if (rows.length) return { rows, payload, ok: true };
    const empty = unwrapItems(payload);
    if (Array.isArray(empty)) return { rows: [], payload, ok: true };
  }
  return { rows: [], payload: null, ok: false };
}
...
async function runArcTokenScan() {
  ...
  try {
    ...
    const seen = store.get('arc_token_seen_v5', {});
    ...
    for (const wallet of wallets) {
      walletQueries++;
      const result = await scanWalletTransfers(wallet, cursor);
      ...
      for (const row of result.rows) {
        ...
        newestIndexedBlock = Math.max(newestIndexedBlock, row.block);
        rows.push(row);
      }
    }

    if (!apiWorking) {
      console.warn('[ARC TOKEN] ArcScan filter/token-transfers API unavailable; no token alerts generated');
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, error: 'ArcScan filter token-transfer API unavailable', baselineReady: true };
    }
    ...
    store.set('arc_token_seen_v5', seen);
