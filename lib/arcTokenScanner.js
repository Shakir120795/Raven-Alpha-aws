...
function buildActivityFilter(address, cursor) {
  const params = new URLSearchParams({ address, limit: String(API_PAGE_SIZE) });
  if (cursor > 0) params.set('cursor', String(cursor));
  return `/address/${address}/activity?${params.toString()}`;
}

async function scanWalletActivity(wallet, cursor) {
  const payload = await apiGet(buildActivityFilter(wallet.address, 0), false);
...
async function runArcTokenScan() {
  ...
  try {
    ...
    const seen = store.get('arc_token_seen_v6', {});
    ...
    store.set('arc_token_seen_v6', seen);
