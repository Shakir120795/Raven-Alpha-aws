...
function buildActivityPath(address) {
  // ArcScan: address is a path parameter. Do not repeat it in the query string.
  return `/address/${address.toLowerCase()}/activity?limit=${API_PAGE_SIZE}`;
}

async function scanWalletActivity(wallet, cursor) {
  const payload = await apiGet(buildActivityPath(wallet.address), false);
  if (!payload) return { ok: false, rows: [] };
  return { ok: true, rows: unwrapItems(payload), payload };
}
...
async function runArcTokenScan() {
  ...
  try {
    ...
    const seen = store.get('arc_token_seen_v7', {});
    ...
    for (const wallet of wallets) {
      walletQueries++;
      const result = await scanWalletActivity(wallet, cursor);
    ...
    if (!apiWorking) {
      console.warn('[ARC TOKEN] ArcScan address activity API unavailable; no token alerts generated');
      return { walletsScanned: wallets.length, tokenAlerts: 0, tokenTransfers: 0, error: 'ArcScan address activity API unavailable', baselineReady: true };
    }
    ...
    store.set('arc_token_seen_v7', seen);
    ...
    return { walletsScanned: wallets.length, tokenAlerts, tokenTransfers: transferCount, buyCandidates: buyCandidateCount, confirmedBuys: buyConfirmedCount, baselineReady: true };
  } finally {
    running = false;
  }
}
